...
def load_cfg() -> UGCConfig:
    ...
    return UGCConfig(
        ...
        # IMPORTANTE: esto es MAX (tope). El clip puede ser menor si el video dura menos.
...
def seconds_from_filename(filename: str) -> Optional[int]:
    """
    Si el nombre tiene __s10, devuelve 10.
    Ej: clip__s5.mp4, gameplay__s15_highlight.mp4
    """
...
def probe_video_duration_seconds(path: str) -> int:
    """
    Duración aproximada en segundos (entero) usando ffprobe (incluido con ffmpeg).
    """
    cmd = [
        "ffprobe",
        "-v", "error",
...
# ----------------------------
# ffmpeg: adaptar mp4 a Reel vertical
# ----------------------------

def build_vertical_reel(input_mp4: str, output_mp4: str, seconds: int) -> None:
    # scale para cubrir 1080x1920 y luego crop centrado (sin bordes)
...
def default_state() -> Dict[str, Any]:
    return {
        "version": 1,
        "processed": {},  # inbox_key -> {etag, processed_at, reel_key, ig_media_id}
        "failed": {},     # inbox_key -> {etag, failed_at, error}
        "last_run_at": None,
    }
...
def run_mode_b() -> None:
    ...
    print("IG enabled:", cfg.enable_ig_publish)
    print("UGC max seconds (cap):", cfg.reel_seconds_max)
    print("UGC max per run:", cfg.max_per_run)
    ...
    for obj in items:
        ...
        try:
            ...
            # duración (auto) + override por nombre
            ...
            print("[UGC] Reel listo:", local_out)
